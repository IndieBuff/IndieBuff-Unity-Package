    ...
    public class FileBasedContextSystem
    {
        ...
        private double CalculateTextScore(SceneNode node, string[] queryTerms)
        {
            ...
            foreach (var term in queryTerms)
            {
                ...
                double nameScore = FuzzyMatch.Calculate(node.Name.ToLower(), term);
                double tagScore = !string.IsNullOrEmpty(node.GameObject.tag) && node.GameObject.tag != "Untagged"
                    ? FuzzyMatch.Calculate(node.GameObject.tag.ToLower(), term)
                    : 0;
                double layerScore = FuzzyMatch.Calculate(LayerMask.LayerToName(node.GameObject.layer).ToLower(), term);
                double componentScore = node.Components
                    .Where(c => c != null)
                    .Max(c => FuzzyMatch.Calculate(c.GetType().Name.ToLower(), term));
